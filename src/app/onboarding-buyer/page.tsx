...
const questions = [
  { label: 'Business Type', type: 'text', name: 'businessType' },
  { label: 'Acquisition Budget', type: 'number', name: 'budget' },
  { label: 'Preferred Industry', type: 'text', name: 'industry' },
  { label: 'Location', type: 'text', name: 'location' },
  { label: 'Deal Urgency', type: 'text', name: 'urgency' },
];
...
export default function OnboardingBuyer() {
  ...
  if (submitted) {
    return (
      <main style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#111' }}>
        <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 16px rgba(0,0,0,0.08)', padding: '2rem', maxWidth: 400, textAlign: 'center' }}>
          <h2 style={{ color: '#00796b' }}>Thank you for completing the Buyer Onboarding!</h2>
          <p style={{ color: '#00796b', marginTop: '1rem' }}>Your information has been submitted. You can now browse sellers and get matched.</p>
          <button onClick={handleRestart} style={{ marginTop: '2rem', background: '#b2dfdb', color: '#00796b', borderRadius: 8, padding: '0.5rem 1.5rem', border: 'none' }}>Restart Questionnaire</button>
        </div>
      </main>
    );
  }
    ...
    <main style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#111' }}>
      ...
      <div style={{ marginBottom: '1.5rem', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <h2 style={{ color: '#00796b', margin: 0, fontSize: '1.25rem', fontWeight: 600 }}>Buyer Onboarding</h2>
          </div>
        </div>
